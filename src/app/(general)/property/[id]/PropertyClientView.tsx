...
// --- START: Co-located Helper Components and Functions ---
    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-grow p-4 overflow-y-auto">
          {activeImage && (<div className="relative w-full aspect-[16/10] mb-4 rounded-md overflow-hidden bg-gray-200"><CldImage src={activeImage.publicId || activeImage.url} alt={activeImage.alt || `Image of ${title}`} layout="fill" objectFit="contain" /></div>)}
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">{images.map((image, index) => (<div key={image.publicId || index} className={`relative aspect-square rounded-md overflow-hidden cursor-pointer border-2 ${activeImage?.url === image.url ? 'border-blue-500' : 'border-transparent hover:border-blue-300'}`} onClick={() => setActiveImage(image)}><CldImage src={image.publicId || image.url} alt={image.alt || `Thumbnail ${index + 1}`} layout="fill" objectFit="cover" /></div>))}</div>
        </div>
      </div>
    </div>
  );
...
const getDatesInRange = (startDate: Date, endDate: Date): string[] => {
    const dates: string[] = []; const currentDate = new Date(startDate);
    while (currentDate < endDate) { dates.push(currentDate.toISOString().split('T')[0]); currentDate.setDate(currentDate.getDate() + 1); }
    return dates;
};

const getPrice = (priceGroup: PricingByMealPlan | DiscountedPricingByMealPlan | undefined, mealPlan: keyof PricingByMealPlan): number => {
    if (priceGroup && typeof priceGroup === 'object' && mealPlan in priceGroup) { const price = (priceGroup as any)[mealPlan]; return typeof price === 'number' ? price : 0; }
    return 0;
};

const initialPricingState: RoomCategoryPricing = {
    singleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 }, discountedSingleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 },
    doubleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 }, discountedDoubleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 },
    tripleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 }, discountedTripleOccupancyAdultPrice: { noMeal: 0, breakfastOnly: 0, allMeals: 0 },
    child5to12Price: { noMeal: 0, breakfastOnly: 0, allMeals: 0 }, discountedChild5to12Price: { noMeal: 0, breakfastOnly: 0, allMeals: 0 },
};

// --- END: Helper Components and Functions ---


export default function PropertyClientView({ initialPropertyData }: PropertyClientViewProps) {
    ...
    // FIX: "Re-hydrate" the data. Convert date strings from the prop back into Date objects.
    // This is the CRITICAL fix to prevent runtime errors.
    const [property] = useState<Property>(initialPropertyData);

    // The `loading` and `error` states for the initial fetch are no longer needed.
    
    // All other state management for UI remains the same.
    const [activeImage, setActiveImage] = useState<string | null>(property.bannerImage?.url || property.detailImages?.[0]?.url || null);
    ...
    // Helper functions to get check-in and check-out dates from localStorage
    const getCheckInDateFromLocalStorage = (): string | null => {
        if (typeof window === 'undefined') return null;
        return localStorage.getItem('checkInDate');
    };
    const getCheckOutDateFromLocalStorage = (): string | null => {
        if (typeof window === 'undefined') return null;
    ...
    const handleCategoryTitleClick = (category: StoredRoomCategory) => { setModalData({ title: category.title, images: category.categoryImages || [] }); };
    const handleCloseModal = () => { setModalData(null); };

    const currencySymbol = property.costing?.currency === 'INR' ? '₹' : (property.costing?.currency === 'USD' ? '$' : (property.costing?.currency || '$'));

    // --- RETURN STATEMENT ---
    // The JSX is identical to your original component, without the top-level loading/error checks,
    // as the server component guarantees the `property` object exists.
    return (
        <>
