...
export default function ItemEdit({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchItemDetails = useCallback( async () => {
    ...
    try {
      ...
      if (foundCategory === 'properties') {
        setPropertyDetails({
           userId: foundItem.userId,
                    ...
                    startDate: foundItem?.startDate || new Date().toISOString(),
                    endDate: foundItem?.endDate || new Date().toISOString(),
                    costing: {
      ...
      // console.log( "PProperty FoundItem Testing: " , foundItem );
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="mb-6">
        <Button variant="outline" onClick={() => router.push(`/admin/dashboard/${item.id}`)}>
