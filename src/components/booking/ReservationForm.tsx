...
const Stepper = () => (
    <div className="flex items-center justify-between max-w-lg mx-auto mb-6">
        <div className="flex items-center text-[#005A9C]">
            <CheckCircle className="w-6 h-6 mr-2" />
            <span className="font-semibold">Your Selection</span>
        </div>
        <div className="flex-1 border-t-2 border-gray-300 mx-4"></div>
        <div className="flex items-center text-[#005A9C] font-bold">
            <span className="flex items-center justify-center w-6 h-6 mr-2 border-2 border-[#005A9C] rounded-full text-sm">2</span>
...
export default function ReservationForm({ propertyId }: { propertyId: string }) {
    ...
    if (loading) { return <div className="flex justify-center items-center min-h-screen bg-gray-100"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#005A9C] mx-auto"></div></div>; }
    if (error || !reservationDetails) { return <div className="container mx-auto px-4 py-16 text-center"><h2 className="text-2xl font-bold text-red-600 mb-4">{error || 'Could not load reservation.'}</h2><button onClick={() => router.push('/properties')} className="mt-4 px-6 py-2 bg-[#005A9C] text-white rounded-md hover:bg-[#005A9C]">Find Properties</button></div>; }
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-1 space-y-4 lg:sticky top-5 self-start">
                            <div className="border border-gray-300 rounded-md p-4 space-y-4">
                                ...
                                {propertyRating && <div className="flex items-center gap-2"><div className="bg-[#005A9C] text-white font-bold text-sm px-2 py-1 rounded-md">{propertyRating.toFixed(1)}</div><span className="font-semibold">Good</span><span className="text-xs text-gray-500">1,012 reviews</span></div>}
                            ...
                            <div className="border border-gray-300 rounded-md p-4 space-y-3">
                                ...
                                <hr/>
                                <div><p className="font-semibold">You selected</p><p className="text-[#005A9C] font-bold text-sm">{roomInstances.length} room{roomInstances.length > 1 && 's'} for {globalGuestCount} guests</p><button onClick={() => router.back()} className="text-[#005A9C] text-sm font-semibold hover:underline">Change your selection</button></div>
                            </div>
                            <div className="border border-gray-300 rounded-md p-4">
                                ...
                                <button onClick={() => setShowPriceDetails(!showPriceDetails)} className="text-[#005A9C] text-sm font-semibold hover:underline flex items-center mt-2">Price information {showPriceDetails ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            {isSignedIn && user && <div className="bg-[#005A9C] border border-[#005A9C] rounded-md p-4 flex items-center space-x-4"><Image src={user.imageUrl} alt="user avatar" width={40} height={40} className="rounded-full" /><p>You are signed in as <span className="font-bold">{user.emailAddresses[0].emailAddress}</span></p></div>}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center space-x-6">
                                            <label className="flex items-center cursor-pointer"><input type="radio" name="bookingFor" value="self" checked={bookingFor === 'self'} onChange={() => setBookingFor('self')} className="h-4 w-4 text-[#005A9C] focus:ring-[#005A9C] border-gray-300" /> <span className="ml-2">I&apos;m the main guest</span></label>
                                            <label className="flex items-center cursor-pointer"><input type="radio" name="bookingFor" value="someone_else" checked={bookingFor === 'someone_else'} onChange={() => setBookingFor('someone_else')} className="h-4 w-4 text-[#005A9C] focus:ring-[#005A9C] border-gray-300" /> <span className="ml-2">I&apos;m booking for someone else</span></label>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center space-x-6">
                                            <label className="flex items-center cursor-pointer"><input type="radio" name="travelingFor" value="work" checked={travelingFor === 'work'} onChange={() => setTravelingFor('work')} className="h-4 w-4 text-[#005A9C] focus:ring-[#005A9C] border-gray-300" /> <span className="ml-2">Yes</span></label>
                                            <label className="flex items-center cursor-pointer"><input type="radio" name="travelingFor" value="leisure" checked={travelingFor === 'leisure'} onChange={() => setTravelingFor('leisure')} className="h-4 w-4 text-[#005A9C] focus:ring-[#005A9C] border-gray-300" /> <span className="ml-2">No</span></label>
                                        </div>
                                    </div>
                            ...
                            <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
                                <div>
                                    ...
                                    <div className="mt-4 border border-gray-200 rounded-md divide-y divide-gray-200">
                                        <label className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50"><div className="flex items-center"><Bus size={24} className="mr-4 text-[#005A9C]"/><p className="font-semibold">I&apos;m interested in an airport shuttle</p></div><input type="checkbox" checked={wantsAirportShuttle} onChange={() => setWantsAirportShuttle(!wantsAirportShuttle)} className="h-5 w-5 rounded border-gray-300 text-[#005A9C] focus:ring-[#005A9C]"/></label>
                                        <label className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50"><div className="flex items-center"><Car size={24} className="mr-4 text-[#005A9C]"/><p className="font-semibold">I&apos;m interested in renting a car</p></div><input type="checkbox" checked={wantsCarRental} onChange={() => setWantsCarRental(!wantsCarRental)} className="h-5 w-5 rounded border-gray-300 text-[#005A9C] focus:ring-[#005A9C]"/></label>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <label className="flex items-center mt-2 cursor-pointer"><input type="checkbox" checked={wantsRoomsTogether} onChange={() => setWantsRoomsTogether(!wantsRoomsTogether)} className="h-4 w-4 rounded border-gray-300 text-[#005A9C] mr-2"/>I want rooms close to each other (if available)</label>
                                </div>
                                <div>
                            ...
                            <div className="text-right">
                                <RazorpayPaymentButton
                                    className="bg-[#005A9C] text-white font-bold py-3 px-6 rounded-md hover:bg-[#005A9C] transition-colors text-lg"
            ...
            {bookingConfirmed && (
                <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-[110] backdrop-blur-sm">
                    <div className="bg-white rounded-lg max-w-md w-full p-7 text-center shadow-xl">
                        ...
                        <button onClick={() => router.push('/customer/bookings')} className="w-full bg-[#005A9C] text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-[#005A9C]">View My Bookings</button>
                    </div>
                </div>
            )}
        </>
