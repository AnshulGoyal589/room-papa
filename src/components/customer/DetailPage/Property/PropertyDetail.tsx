...
import {
    ...
    AlertTriangle, Bed, Utensils, UserCheck, Home, X, HelpCircle,
} from 'lucide-react';
...
const getPriceDetails = (basePrice: number, discountedPrice: number) => {
    const isDiscounted = discountedPrice > 0 && discountedPrice < basePrice;
    const finalBasePrice = isDiscounted ? discountedPrice : basePrice;
    const originalPriceForCalc = isDiscounted ? basePrice : undefined;
    return { finalBasePrice, originalPriceForCalc, isDiscounted };
};
...
const checkCategoryAvailability = (
    ...
    if (cat.availability && cat.availability.length > 0) {
        const isWithinAvailablePeriod = cat.availability.some(period => {
        ...
        if (!isWithinAvailablePeriod) {
            return { available: false, message: `Room type "${cat.title}" is not available for the selected dates. Please check its seasonal availability.` };
        }
    }

    if (cat.unavailableDates && cat.unavailableDates.length > 0) {
        const isBlocked = bookingDateRange.some(dateStr => {
            const currentDate = new Date(dateStr); currentDate.setUTCHours(12);
            return cat.unavailableDates.some(period => {
                const periodStart = new Date(period.startDate); periodStart.setUTCHours(12);
                const periodEnd = new Date(period.endDate); periodEnd.setUTCHours(12);
                return currentDate >= periodStart && currentDate <= periodEnd;
            });
        });
        if (isBlocked) {
            return { available: false, message: `Room type "${cat.title}" is unavailable during your selected dates due to a blackout period.` };
        }
    }
...
const calculateAverageHikePerNight = (
    cat: RoomCategory,
    occupancyType: keyof HikePricingByOccupancy | null,
    selectedMealPlan: keyof PricingByMealPlan,
    bookingDateRange: string[]
): number => {
    const { seasonalHike: hike } = cat;
    if (!hike?.startDate || !hike.endDate || !hike.hikePricing || !occupancyType || bookingDateRange.length === 0) {
        return 0;
    }

    const getHikeAmount = (hikePricing: HikePricingByOccupancy): number => {
        const hikeGroup = hikePricing[occupancyType];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (hikeGroup && typeof hikeGroup === 'object' && selectedMealPlan in hikeGroup) ? (hikeGroup as any)[selectedMealPlan] : 0;
    };

    const hikeStartDate = new Date(`${hike.startDate}T00:00:00`);
    const hikeEndDate = new Date(`${hike.endDate}T23:59:59`);
    const hikeAmountPerNight = getHikeAmount(hike.hikePricing);

    if (hikeAmountPerNight === 0) return 0;
    ...
    return totalHikeAmount / bookingDateRange.length;
};
...
export default function PropertyDetailPage({ property }: { property: Property | null }) {
    ...
    useEffect(() => {
        ...
        if (storedPrefsStr) {
            try {
                ...
                setCheckInDate(storedCheckIn ? new Date(storedCheckIn) : null);
                setCheckOutDate(storedCheckOut ? new Date(storedCheckOut) : null);
                ...
                if (parsedPrefs.propertyId === property._id?.toString()) {
                    ...
                    // FIXED: Ensured adultCount is parsed as an integer
                    setAdultCount(parseInt(parsedPrefs.adultCount || localStorage.getItem('adults') || '1', 10));
                    setChildCount(parseInt(parsedPrefs.childCount || localStorage.getItem('children') || '0', 10));
        ...
        // Default fallbacks
        setAdultCount(localStorage.getItem('adults') ? parseInt(localStorage.getItem('adults')!, 10) : 1);
        setChildCount(localStorage.getItem('children') ? parseInt(localStorage.getItem('children')!, 10) : 0);
        if (availableBookingModels.length === 1) {
    ...
    // --- Core Logic for Generating Room Offers ---
    const displayableRoomOffers = useMemo((): DisplayableRoomOffer[] => {
        ...
        const categoriesToProcess = property.categoryRooms.filter(
            cat => !cat.pricingModel || cat.pricingModel === selectedBookingModel
        );
        ...
        categoriesToProcess.forEach(cat => {
            const { available } = checkCategoryAvailability(cat, bookingStart, bookingEnd, bookingDateRange);
            if (!available) return;

            
            const createOffer = (
                priceDetails: ReturnType<typeof getPriceDetails>,
                hikePerNight: number,
                offerIdentifier: string,
                occupancyDetails: { intendedAdults: number; intendedChildren: number; guestCapacity: number; }
            ) => ({
                offerId: `${cat.id}_${offerIdentifier}`, categoryId: cat.id, categoryTitle: cat.title, bedConfiguration: cat.bedConfiguration,
                roomSpecificAmenities: cat.roomSpecificAmenities, maxPhysicalRoomsForCategory: cat.qty || 1, intendedAdults: occupancyDetails.intendedAdults,
                intendedChildren: occupancyDetails.intendedChildren, guestCapacityInOffer: occupancyDetails.guestCapacity,
                pricePerNight: priceDetails.finalBasePrice + hikePerNight,
                originalPricePerNight: priceDetails.originalPriceForCalc ? priceDetails.originalPriceForCalc + hikePerNight : undefined,
                isDiscounted: priceDetails.isDiscounted, currency: cat.currency, roomSize: cat.roomSize || "Unknown",
                categoryActivities: cat.categoryActivities, categoryFacilities: cat.categoryFacilities,
            });
            ...
            if (selectedBookingModel === 'perUnit') {
                ...
                const priceDetails = getPriceDetails(basePrice, discountedPrice);
                const averageHikePerNight = calculateAverageHikePerNight(cat, null, selectedMealPlan, bookingDateRange);
                const totalOccupancy = cat.totalOccupancy || 1;
                offers.push(createOffer(priceDetails, averageHikePerNight, 'unit',
                    { intendedAdults: totalOccupancy, intendedChildren: 0, guestCapacity: totalOccupancy }
                ));
            }
            
            if (selectedBookingModel === 'perOccupancy') {
                const occupancyLevels = [
                    { numAdults: 1, type: 'singleOccupancyAdultHike', base: cat.pricing.singleOccupancyAdultPrice, disc: cat.pricing.discountedSingleOccupancyAdultPrice },
                    { numAdults: 2, type: 'doubleOccupancyAdultHike', base: cat.pricing.doubleOccupancyAdultPrice, disc: cat.pricing.discountedDoubleOccupancyAdultPrice },
                    { numAdults: 3, type: 'tripleOccupancyAdultHike', base: cat.pricing.tripleOccupancyAdultPrice, disc: cat.pricing.discountedTripleOccupancyAdultPrice },
                ] as const;

                occupancyLevels.forEach(({ numAdults, type, base, disc }) => {
                    if (MAX_OCCUPANTS_PER_ROOM < numAdults) return;
                    const basePrice = getPrice(base, selectedMealPlan);
                    if (basePrice === 0) return;
                    const discountedPrice = getPrice(disc, selectedMealPlan);
                    const priceDetails = getPriceDetails(basePrice, discountedPrice);
                    const averageHikePerNight = calculateAverageHikePerNight(cat, type, selectedMealPlan, bookingDateRange);
                    offers.push(createOffer(priceDetails, averageHikePerNight, `${numAdults}guests`,
                        { intendedAdults: numAdults, intendedChildren: 0, guestCapacity: numAdults }
                    ));
                });
            }
        });
    ...
    // --- Total Booking Pricing Calculation (Refactored to be a pure calculation) ---
    const { totalBookingPricing, subtotalNights, serviceCharge, taxesApplied, pricingError } = useMemo(() => {
        const defaultReturn = { totalBookingPricing: 0, subtotalNights: 0, serviceCharge: 0, taxesApplied: 0, totalCapacityNeeded: 0, pricingError: null };
        ...
        if (selectedBookingModel === 'perUnit' && totalGuests > capacityAvailable) {
            return { ...defaultReturn, pricingError: `Capacity Error: The selected unit capacity (${capacityAvailable} guests) is less than the total guests (${totalGuests}).` };
        }
        
        let childrenPriceComponent = 0;
        if (selectedBookingModel === 'perOccupancy' && childCount > 0) {
            ...
            if (remainingChildren > 0) {
                return { ...defaultReturn, pricingError: `Occupancy Error: Cannot assign all ${childCount} children to the selected rooms due to occupancy limits.` };
            }
        }
        ...
        if (pricePerNight <= 0 && totalSelectedPhysicalRooms > 0) {
            return { ...defaultReturn, pricingError: "Price Calculation Error: Could not determine a valid nightly rate for the selection." };
        }
        ...
        return { totalBookingPricing: currentTotal, subtotalNights: currentSubtotalNights, serviceCharge: currentServiceCharge, taxesApplied: currentTaxes, totalCapacityNeeded: capacityAvailable, pricingError: null };
    ...
    const handleCheckInChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const newCheckIn = e.target.value ? validateDate(e.target.value) : null;
    ...
    const handleCheckOutChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        let newCheckOut = e.target.value ? validateDate(e.target.value) : null;
    ...
    const handleOfferQuantityChange = useCallback((offerId: string, quantity: number) => {
        if (!checkInDate || !checkOutDate || !property?.categoryRooms) return;
        
        const bookingStart = new Date(checkInDate); bookingStart.setHours(0,0,0,0);
        const bookingEnd = new Date(checkOutDate); bookingEnd.setHours(0,0,0,0);
        const bookingDates = getDatesInRange(checkInDate, checkOutDate);

        let newSelectedOffers = { ...selectedOffers };

        if (selectedBookingModel === 'perUnit') {
            const isSelecting = quantity > 0;
            if (isSelecting && totalSelectedPhysicalRooms > 0 && !selectedOffers[offerId]) {
            ...
            newSelectedOffers = isSelecting ? { [offerId]: 1 } : {};
        } else { // Per Occupancy Logic
        ...
        if (bookingError && !bookingError.startsWith("Selection Error:")) setBookingError(null); 

        // Centralized availability check for all selected categories
        let finalAvailabilityError: string | null = null;
        const involvedCategoryIds = new Set(Object.keys(newSelectedOffers).map(offerId => offerId.split('_')[0]));
        for (const catId of Array.from(involvedCategoryIds)) {
            const category = property.categoryRooms.find(c => c.id === catId);
            if (category) {
                const { available, message } = checkCategoryAvailability(category, bookingStart, bookingEnd, bookingDates);
                if (!available) {
                    finalAvailabilityError = message;
                    break;
                }
            }
        }
        setAvailabilityError(finalAvailabilityError);

    }, [checkInDate, checkOutDate, selectedOffers, totalSelectedPhysicalRooms, property?.categoryRooms, selectedBookingModel, bookingError]);
    ...
    const handleBookNowOrReserveClick = () => {
        ...
        if (availabilityError) { setBookingError(availabilityError); return; }
        if (pricingError) { setBookingError(pricingError); return; }
        if (totalBookingPricing <= 0) { setBookingError("Could not calculate a valid price. Please check dates and selections."); return; }
        
        setBookingError(null);
        
        const reservationData = {
            propertyId: property?._id, checkInDate: checkInDate.toISOString(), checkOutDate: checkOutDate.toISOString(),
            days, adultCount, childCount, selectedOffers, selectedMealPlan, selectedBookingModel, 
    ...
    return (
        <>
            {/* The entire JSX structure remains unchanged from your original code */}
                ...
                <div className="container mx-auto px-2 sm:px-4 lg:max-w-7xl py-16">
                    ...
                    <div className="bg-white rounded-md border border-gray-300 mb-6">
                        {needsModelSwitch && (
                            <div className="p-4 border-b border-gray-200 bg-blue-50">
                                ...
                                <div className="flex space-x-4">
                                    <button onClick={() => handleBookingModelSwitch('perOccupancy')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors border ${selectedBookingModel === 'perOccupancy' ? 'bg-[#003c95] text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                                        <UsersIcon size={14} className="inline mr-1 mb-0.5" /> Book by Occupancy
                                    </button>
                                    <button onClick={() => handleBookingModelSwitch('perUnit')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors border ${selectedBookingModel === 'perUnit' ? 'bg-[#003c95] text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
                        ...
                        <div className="bg-gray-50 p-4 border-b border-gray-200">
                            <h2 className="text-xl font-bold text-gray-800 mb-3">{selectedBookingModel === 'perUnit' ? 'Select your unit' : 'Select your rooms'}</h2>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                                <div><label htmlFor="checkin-date-table" className="block text-xs font-medium text-gray-500 mb-1">Check-in</label><input id="checkin-date-table" type="date" value={checkInDate ? checkInDate.toISOString().split('T')[0] : ''} onChange={handleCheckInChange} className="w-full p-2 border rounded-md text-sm shadow-sm" min={todayStr} max={maxDateStr} /></div>
                                <div><label htmlFor="checkout-date-table" className="block text-xs font-medium text-gray-500 mb-1">Check-out</label><input id="checkout-date-table" type="date" value={checkOutDate ? checkOutDate.toISOString().split('T')[0] : ''} onChange={handleCheckOutChange} className="w-full p-2 border rounded-md text-sm shadow-sm" min={checkInDate ? new Date(checkInDate.getTime() + 86400000).toISOString().split('T')[0] : todayStr} max={maxDateStr} disabled={!checkInDate} /></div>
                                <div><label htmlFor="adult-count-selector" className="block text-xs font-medium text-gray-500 mb-1">Adults</label><select id="adult-count-selector" value={adultCount} onChange={e => setAdultCount(parseInt(e.target.value))} className="w-full p-2 border rounded-md text-sm">{[...Array(10)].map((_, i) => <option key={i + 1} value={i + 1}>{i + 1}</option>)}</select></div>
                                <div><label htmlFor="child-count-selector" className="block text-xs font-medium text-gray-500 mb-1">Children</label><select id="child-count-selector" value={childCount} onChange={e => setChildCount(parseInt(e.target.value))} className="w-full p-2 border rounded-md text-sm">{[...Array(6)].map((_, i) => <option key={i} value={i}>{i}</option>)}</select></div>
                            </div>
                            <div className="mt-4"><label className="block text-xs font-medium text-gray-500 mb-1.5">Preferred Meal Plan (for all {selectedBookingModel === 'perUnit' ? 'units' : 'rooms'}):</label><div className="flex flex-wrap gap-2">{(['noMeal', 'breakfastOnly', 'allMeals'] as (keyof PricingByMealPlan)[]).map(plan => { const labelText = { noMeal: 'Room Only', breakfastOnly: 'Breakfast', allMeals: 'All Meals' }[plan]; return (<label key={`meal-plan-table-${plan}`} className={`flex items-center px-2.5 py-1 border rounded-md cursor-pointer transition-colors text-xs ${selectedMealPlan === plan ? 'bg-[#003c95] border-[#003c95] ring-1 ring-[#003c95]' : 'border-gray-300 hover:bg-gray-100'}`}><input type="radio" name="mealPlanTable" value={plan} checked={selectedMealPlan === plan} onChange={(e) => setSelectedMealPlan(e.target.value as keyof PricingByMealPlan)} className="h-3 w-3 text-[#white] border-gray-300 focus:ring-[#003c95] mr-1.5" /><span className={`${selectedMealPlan === plan ? 'text-white font-semibold' : 'text-gray-600'}`}>{labelText}</span></label>); })}</div></div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse">
                                ...
                                <tbody className="block lg:table-row-group lg:divide-y lg:divide-gray-200 border-b border-gray-300">
                                    {(!checkInDate || !checkOutDate || days <= 0) && (<tr className="block lg:table-row"><td colSpan={6} className="block lg:table-cell text-center py-8 text-gray-500 font-semibold">Please select your dates to see available rooms and prices.</td></tr>)}
                                    {(checkInDate && checkOutDate && days > 0 && displayableRoomOffers.length === 0) && (<tr className="block lg:table-row"><td colSpan={6} className="block lg:table-cell text-center py-8 text-gray-500">No {selectedBookingModel === 'perUnit' ? 'units' : 'rooms'} available for the selected dates or meal plan in this model. Please try different options.</td></tr>)}
                                    {displayableRoomOffers.map((offer, overallOfferIndex) => {
                                        const category = property.categoryRooms?.find(cat => cat.id === offer.categoryId); if (!category) return null;
                                        ...
                                        const maxSelectableForThisOffer = offer.maxPhysicalRoomsForCategory;
                                        return (
                                            <tr key={offer.offerId} className={`block p-4 border rounded-lg mb-4 lg:p-0 lg:table-row lg:border-none lg:mb-0 ${currentQtySelected > 0 ? 'bg-blue-50' : 'bg-white'}`}>
                                                {(selectedBookingModel === 'perUnit' || offerIndexInCategory === 0) && (
                                                    <td className="block border-b pb-4 mb-4 lg:border-b-0 lg:pb-0 lg:mb-0 lg:table-cell lg:px-4 lg:py-3 lg:align-top lg:border-r" rowSpan={selectedBookingModel === 'perOccupancy' ? offersForThisCategory.length : 1}>
                                                        <div className="relative w-full aspect-[4/3] rounded-lg overflow-hidden cursor-pointer group mb-3 shadow-sm" onClick={() => handleCategoryTitleClick(category)}><CldImage src={category.categoryImages?.[0]?.url || '/images/placeholder-property.png'} alt={`Image of ${category.title}`} layout="fill" objectFit="cover" />{category.categoryImages && category.categoryImages.length > 0 && <div className="absolute bottom-2 left-2 bg-white/90 text-gray-900 text-xs font-bold px-3 py-1.5 rounded-full shadow-lg"> {category.categoryImages.length} PHOTOS </div>}</div>
                                                        <h3 className="font-bold text-gray-800 text-xl">{offer.categoryTitle}</h3>
                                                        <div className="mt-3 p-1.5 bg-green-50 border border-green-200 rounded-sm text-xs text-green-700 flex items-center">{selectedBookingModel === 'perUnit' ? (<><Home size={14} className="mr-1.5 shrink-0" /> Unit capacity: {offer.guestCapacityInOffer} guests</>) : (<><UserCheck size={14} className="mr-1.5 shrink-0" /> Recommended for {offer.intendedAdults} adult{offer.intendedAdults > 1 ? 's' : ''}</>)}</div>
                                                    </td>
                                                )}
                                                <td className="block py-2 border-b lg:border-b-0 lg:table-cell lg:px-3 lg:py-3 lg:align-top lg:text-center lg:border-r">{selectedBookingModel === 'perUnit' ? (<p className="text-lg font-bold text-gray-700 mt-1">{offer.guestCapacityInOffer}</p>) : (<div className="flex items-center lg:justify-center">{[...Array(offer.intendedAdults)].map((_, i) => <UsersIcon key={i} size={18} className="text-gray-600" />)}</div>)}</td>
                                                ...
                                                <td className="block py-2 border-b lg:border-b-0 lg:table-cell lg:px-3 lg:py-3 lg:align-top text-xs space-y-1.5 lg:border-r"><span className="text-sm font-semibold text-gray-500 lg:hidden">Your choices include:</span><div className="mt-1 lg:mt-0"><p className="flex items-center font-semibold"> {selectedMealPlan === 'breakfastOnly' ? <CoffeeIconLucide size={14} className="mr-1.5 shrink-0 text-gray-600" /> : selectedMealPlan === 'allMeals' ? <Utensils size={14} className="mr-1.5 shrink-0 text-gray-600" /> : <Bed size={14} className="mr-1.5 shrink-0 text-gray-600" />} <span className="flex-grow"> {selectedMealPlan === 'breakfastOnly' ? 'Very good breakfast included' : selectedMealPlan === 'allMeals' ? 'Breakfast & dinner included' : 'Room only accommodation'} </span> <HelpCircle size={13} className="ml-1 text-gray-400 hover:text-gray-600 cursor-pointer flex-shrink-0" aria-label={`This offer is for ${selectedMealPlan === 'noMeal' ? 'room only' : selectedMealPlan === 'breakfastOnly' ? 'room with breakfast' : 'room with all meals'}.`} /></p>{property.reservationPolicy?.some(p => ['Free Cancellation', 'Flexible', 'Moderate'].includes(p)) && (<p className="flex items-center text-green-600"><CheckCircle size={14} className="mr-1.5 shrink-0" /> Free cancellation <span className="text-gray-500 ml-1 text-[10px]">(before specific date/time - check details)</span></p>)}{property.reservationPolicy?.includes('Pay at Property') && (<p className="flex items-center text-green-600"><CheckCircle size={14} className="mr-1.5 shrink-0" /> No prepayment needed <span className="text-gray-500 ml-1 text-[10px]"> – pay at the property</span></p>)}{property.reservationPolicy?.some(p => ['Non-Refundable', 'Strict'].includes(p)) && !property.reservationPolicy?.some(p => ['Free Cancellation', 'Flexible', 'Moderate'].includes(p)) && (<p className="flex items-center text-red-600 font-semibold"><UsersIcon size={14} className="mr-1.5 shrink-0" /> Non-Refundable</p>)}</div></td>
                                                <td className="block py-2 border-b lg:border-b-0 lg:table-cell lg:px-3 lg:py-3 lg:align-top lg:text-center lg:border-r">{selectedBookingModel === 'perUnit' ? (currentQtySelected > 0 ? (<button onClick={() => handleOfferQuantityChange(offer.offerId, 0)} className="bg-red-500 text-white font-semibold py-1.5 px-3 rounded text-sm hover:bg-red-600 flex items-center justify-center mx-auto" disabled={!!availabilityError}><X size={16} className="mr-1" /> Remove Unit</button>) : (<button onClick={() => handleOfferQuantityChange(offer.offerId, 1)} className={`font-semibold py-1.5 px-3 rounded text-sm mx-auto ${days <= 0 || !!availabilityError || totalSelectedPhysicalRooms > 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-500 text-white hover:bg-green-600'}`} disabled={days <= 0 || !!availabilityError || totalSelectedPhysicalRooms > 0}>Select Unit</button>)) : (<select value={currentQtySelected} onChange={(e) => handleOfferQuantityChange(offer.offerId, parseInt(e.target.value))} className="p-1.5 border rounded text-sm w-20" disabled={days <= 0 || maxSelectableForThisOffer < 0 || !!availabilityError}>{[...Array(Math.max(0, maxSelectableForThisOffer) + 1)].map((_, i) => <option key={i} value={i}>{i}</option>)}</select>)}</td>
                                                {overallOfferIndex === 0 && (
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white p-3 border-t shadow-lg lg:hidden z-40">
                {totalSelectedPhysicalRooms > 0 && days > 0 && (<div className="text-right mb-2"><span className="text-sm">Total for {days} night{days > 1 && 's'}: </span><span className="text-xl font-bold">{currencySymbol} {totalBookingPricing.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>)}
