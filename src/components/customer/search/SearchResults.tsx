...
const getRatingDescription = (rating?: number): { text: string; className: string } => {
  ...
  if (rating >= 8.5) return { text: "Fabulous", className: "text-[#005A9C]" };
  if (rating >= 8.0) return { text: "Very Good", className: "text-[#005A9C]" };
...
export default function SearchResults() {
      ...
      <div className="flex-grow p-4 flex flex-col">
        <div className="flex justify-between items-start">
            <Link href={`/property/${property._id}?checkIn=${checkInQuery || ''}&checkOut=${checkOutQuery || ''}&adults=${adultsQuery}&children=${childrenQuery}`} className="block mb-1">
                <h3 className="text-xl md:text-2xl font-bold text-[#005A9C] hover:text-[#005A9C] transition-colors line-clamp-2">{property.title || "Untitled Property"}</h3>
            </Link>
            ...
            {(property.totalRating || property.propertyRating) && (
              <div className="sm:hidden flex flex-col items-end ml-2">
                ...
                <div className="bg-[#005A9C] text-white text-sm font-bold px-2 py-0.5 rounded">
        ...
        <div className="text-xs text-gray-600 mb-2">
            <span className="text-[#005A9C] hover:underline">{property.location?.city}</span>
            <span className="mx-1 text-gray-400">•</span>
            <span className="text-[#005A9C] hover:underline">Show on map</span>
        ...
        <div className="w-full text-center sm:text-right mb-3 sm:mb-0">
          {(property.totalRating || property.propertyRating) && (
            <div className="hidden sm:flex items-center justify-end gap-2 mb-1">
              ...
              <div className="bg-[#005A9C] text-white text-base font-bold px-2 py-1 rounded h-fit">
                {(property.totalRating || property.propertyRating)?.toFixed(1)}
              </div>
            </div>
          )}
        ...
        <div className="w-full text-center sm:text-right">
          ...
          {property.categoryRooms && property.categoryRooms.length > 0 && property.categoryRooms[0]?.pricing?.discountedDoubleOccupancyAdultPrice?.noMeal !== undefined && (
           ...
           <Link 
            ...
            className="mt-2.5 block bg-[#005A9C] hover:bg-[#005A9C] text-white font-semibold py-2.5 px-4 rounded-md text-sm transition-colors w-full flex items-center justify-center"
          >
            See availability
        ...
        <div className="w-full sm:w-2/3 md:flex-grow p-3 sm:p-4 flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between sm:items-start mb-1">
                <Link href={`/customer/trip/${trip._id}`} className="block">
                  <h3 className="text-lg sm:text-xl font-bold text-[#005A9C] hover:text-[#005A9C] transition-colors line-clamp-2">{trip.title || "Adventure Awaits"}</h3>
                </Link>
                {trip.rating !== undefined && trip.rating !== null && (
                  <div className="flex items-center gap-2 mt-1 sm:mt-0 sm:ml-4 flex-shrink-0">
                    ...
                    <div className="bg-[#005A9C] text-white text-sm sm:text-base font-bold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md h-fit">
          ...
          <div className="flex items-center text-xs text-[#005A9C] hover:underline mb-1 sm:mb-2 cursor-pointer" onClick={() => router.push(`/customer/trip/${trip._id}`)}>
          ...
          <div className="mt-auto pt-1 sm:pt-2 text-center sm:text-right">
            ...
            <Link 
              ...
              className="mt-2 inline-block bg-[#005A9C] hover:bg-[#005A9C] text-white font-semibold py-2 px-3 sm:px-5 rounded-md text-xs sm:text-sm transition-colors w-full sm:w-auto"
            >
              View Details
            </Link>
          </div>
  ...
  const renderTravellingCard = (itinerary: Travelling) => {
    ...
    const getTransportationIcon = (type?: TransportationType | string) => { 
      ...
      switch (type) {
        case TransportationType.flight: return <Plane size={iconSize} className={`${iconClasses} text-[#005A9C]`} />;
      ...
      >
        ...
        <div className="w-full sm:w-2/3 md:flex-grow p-3 sm:p-4 flex flex-col">
          <Link href={`/customer/travelling/${itinerary._id}`} className="block mb-1">
            <h3 className="text-lg sm:text-xl font-bold text-[#005A9C] hover:text-[#005A9C] transition-colors line-clamp-2">{itinerary.title || "Custom Itinerary"}</h3>
          </Link>
  ...
  return (
    <div className="bg-[#005A9C]/30 min-h-screen py-6 sm:py-8 px-2 sm:px-4 md:px-6 lg:px-8"> {/* Added light blue background to outer container */}
